...
const renderStats = async (country) => {
  try {
    ...
    fetched.ok ? (fetchedData = true) : (fetchedData = false);
    ...
    liveAlert(
      `Country Not Found! <br><hr> This is probably because you misspelled the name, or the country statistics are not provided by the API we use`,
      'secondary'
    );
